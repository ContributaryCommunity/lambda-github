...
const outputFile = 'repositories.json';
...
function handleRepositoriesResponse(response) {
  console.log('handleRepositoriesResponse.isProduction', isProduction); // eslint-disable-line no-console
...
function getRepositories(projectName, repoType) {
  const path = repoType === 'org'
  ...
  console.log(`GET repositories for ${path}`); // eslint-disable-line no-console
...
function run(event = {}) {
  const { projectName, repoType } = event.queryStringParameters;

  return getRepositories(projectName, repoType)
    .then(handleRepositoriesResponse)
    .catch((error) => {
